...
async def _send_access_message(message: Message, session: AsyncSession, payment) -> None:
    ...
    await message.answer(
        ...
        f"Ссылки для входа:\n{links_text}\n\n"
        "Каждая ссылка одноразовая и ограничена по времени.",
        reply_markup=after_purchase_keyboard(),
    )


@router.callback_query(F.data.startswith("buy_stars:"))
async def buy_stars_handler(callback: CallbackQuery, session: AsyncSession) -> None:
...
async def buy_crypto_handler(callback: CallbackQuery, session: AsyncSession) -> None:
    plan_id = int(callback.data.split(":", maxsplit=1)[1])
    plan = await get_plan_by_id(session, plan_id)
    if plan is None or not plan.is_active:
        await callback.answer("Тариф не найден или отключён", show_alert=True)
...
async def check_crypto_handler(callback: CallbackQuery, session: AsyncSession) -> None:
...
async def successful_payment_handler(message: Message, session: AsyncSession) -> None:
    ...
    if payload.startswith("donate:stars:"):
        amount = payload.split(":")[3] if len(payload.split(":")) > 3 else ""
        thanks_text = "Спасибо за донат ❤️"
        if amount:
            thanks_text = f"Спасибо за донат на {amount} ⭐❤️"
...
async def donate_stars_menu(callback: CallbackQuery) -> None:
    await callback.message.edit_text(
        "Выбери сумму доната в Telegram Stars:",
        reply_markup=donation_amounts_keyboard("stars"),
    )
    await callback.answer()


@router.callback_query(F.data == "donate:crypto")
async def donate_crypto_menu(callback: CallbackQuery) -> None:
    await callback.message.edit_text(
        "Выбери сумму доната через CryptoBot:",
        reply_markup=donation_amounts_keyboard("crypto"),
    )
    await callback.answer()


@router.callback_query(F.data.startswith("donate_stars:"))
async def donate_stars_handler(callback: CallbackQuery) -> None:
    amount = int(callback.data.split(":", maxsplit=1)[1])
    await send_donation_invoice(callback.message, amount)
    await callback.answer(f"Открываю счёт на {amount} ⭐")


@router.callback_query(F.data.startswith("donate_crypto:"))
async def donate_crypto_handler(callback: CallbackQuery) -> None:
    amount = callback.data.split(":", maxsplit=1)[1]
    try:
        pay_url = await create_crypto_donation_invoice(amount)
    except Exception as exc:
        await callback.answer("Не удалось создать донат-счёт", show_alert=True)
        await callback.message.answer(f"Ошибка: <code>{exc}</code>")
        return
